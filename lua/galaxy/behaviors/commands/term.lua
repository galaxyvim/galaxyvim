...
-- buffer_number → terminal
local buf_terms = {}

local buf_term = function(bufnr)
  local Terminal = nvim.require("toggleterm.terminal").Terminal

  if buf_terms[bufnr] then
    return buf_terms[bufnr]
  end
...
local close_term = function(term, bufnr)
  if term and vim.api.nvim_win_is_valid(term.window) then
    ...
    vim.api.nvim_set_current_buf(bufnr) -- set current buffer
    --    vim.api.nvim_set_current_win(windnr)
  end
end
...
vim.api.nvim_create_autocmd("BufEnter", {
  callback = function(args)
    ...
    --    local windnr = vim.api.nvim_get_current_win()
    ...
    if buftype == "" and bufname ~= "" then
      if recent_buf and recent_buf ~= bufnr then
        local prev_term = buf_terms[recent_buf]
        close_term(prev_term, bufnr)
      end
      recent_buf = bufnr
    end
  end,
...
vim.api.nvim_create_user_command("Run", function(opts)
  if not recent_buf or not vim.api.nvim_buf_is_valid(recent_buf) then
  ...
  local file = vim.api.nvim_buf_get_name(recent_buf) -- get filename by last buffer number
  local ftype = vim.bo[recent_buf].filetype
  ...
  local term = buf_term(recent_buf)
...
vim.api.nvim_create_user_command("Term", function(opts)
  recent_buf = recent_buf or vim.api.nvim_get_current_buf()
  if recent_buf then
    local term = buf_term(recent_buf)
