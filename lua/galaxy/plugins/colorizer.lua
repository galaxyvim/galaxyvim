...
M.config = function()
  return {
    ...
    opts = {
      ...
      filetypes = {
        "*", -- filetypes to highlight, "*" for all
        lua = { names = false },
        markdown = {
      ...
      options = {
        ...
        display = {
          mode = nvim.opt.style.colorizer, -- "background"|"foreground"|"underline"|"virtualtext"
          background = {
        ...
        always_update = true, -- update highlights even in unfocused buffers
        debounce_ms = 0, -- debounce highlight updates (ms); 0 = no debounce
      },
    },
    config = function(_, opts)
      local colorizer = nvim.require("colorizer")
